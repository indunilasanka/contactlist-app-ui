...
const Pagination = ({pages, setCurrentPage, currentContacts, filteredContacts}) => {
    ...
    return (
        <div className="clearfix">
            <div className="hint-text">Showing <b>{currentContacts.length}</b> out
                of <b>{filteredContacts.length}</b> entries
            </div>
            <ul className="pagination">
                ...
                <li className={`${currentButton === numOfPages.length ? 'page-item disabled' : 'page-item'}`}>
                    <a href="#" onClick={() => setCurrentButton((next) => next === numOfPages.length ? next : next + 1)}>Next</a></li>
            </ul>
        </div>
    )
}
